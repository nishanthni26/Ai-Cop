...
import { continueWithGoogleDemo, findUser, setCurrentUser } from '../lib/localStore';
        ...
        <div className="glass-card p-8">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Access your scan history.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input value={password} onChange={(event) => setPassword(event.target.value)} type="password" required className="w-full rounded-lg border border-white/10 bg-cyber-dark px-4 py-3 text-white outline-none focus:border-cyber-blue" />
            </div>
            {error && <p className="text-sm text-cyber-red">{error}</p>}
            <button className="btn-cyber w-full" type="submit">Login</button>
          </form>

          <p className="mt-5 text-center text-sm text-gray-400">
            New here? <Link to="/signup" className="text-cyber-blue hover:text-white">Create an account</Link>
          </p>
        </div>
      </div>
    </div>
